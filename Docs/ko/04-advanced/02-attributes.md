...
- `ExcelColumnExtractor` 의 `PrimaryKeyDuplicateChecker` — `[Key]` 가 붙은 컬럼에 한해 추출 단계 값 중복을 검사. 없으면 검사 자체를 스킵.
...
- enum 파라미터에 `[Key]` 를 붙이면 매핑 시 `Enum.IsDefined` 검사가 생략됩니다. enum 을 ID 코드 공간으로 활용하는 [4.3 타입 브랜딩 패턴](./03-type-branding.md) 에서 사용됩니다.
...
|오사용 (로드, 타겟)|`tableSetName` 이 어떤 TableSet 파라미터에도 매칭되지 않으면 `FkTargetNotFound`. `recordColumnName` 이 대상의 `[SingleColumnCollection]` 컬럼이면 `FkTargetIsSingleColumnCollection`. (테이블 적재 직전에 `AggregateException(FkValidationFailed, ...)` 으로 throw)|
...
- 스캐너에는 순수 스키마 규칙인 FK/SFK 동시 부착 검사만 남고, **타겟 존재·타겟 컬럼 존재·타겟이 스칼라인지** 는 모두 `StaticDataManager.LoadAsync` 진입 시점에 검사합니다. 값이 실제로 있는지 (`FkValueNotFound`) 는 그 다음 단계인 `ReferenceValidator` 에서 봅니다.
