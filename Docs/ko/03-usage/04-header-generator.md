...
`StaticDataHeaderGenerator` 는 Record `.cs` 를 읽어서 **Excel 의 헤더 셀에 붙여넣을 수 있는 표준 헤더** 를 출력해 주는 CLI 도구입니다.

단순한 Record 에는 큰 이점이 없지만, 컬렉션이나 중첩 Record 가 들어간 Record 에서는 헤더를 손으로 맞추기 어려워집니다. 이 도구는 그 헤더를 자동으로 조립합니다.
...
출력 파일은 탭(또는 지정한 구분자) 으로 나뉜 한 줄짜리 헤더입니다. **A1 에 적어 둔 헤더 시작 주소** ([3.3](./03-record-to-excel.md) 참조) 가 가리키는 셀에 그대로 붙여넣으면 각 헤더가 한 컬럼씩 자동으로 분리됩니다. 그 아래 행부터 데이터를 채워 넣으면 `ExcelColumnExtractor` 가 읽을 수 있는 시트가 됩니다.
...
예를 들어 A1 에 `B3` 이라고 적어 두기로 합의했다고 합시다. 이때 시트 구성은 다음과 같이 잡습니다.

|셀|용도|
|-|-|
|`A1`|헤더 시작 주소 — `B3` 이라고 적어 둔다|
...
1. C# 작업자와 Excel 작업자가 컬럼 구조를 합의한다 (이 시점에는 Record 가 아직 미확정이어도 됨).
...
5. `ExcelColumnExtractor` 로 CSV 를 추출한다 (CI 단계에서 자동 실행하는 것을 권장).

임시 헤더(B2) 는 추출기가 읽지 않으므로 그대로 둬도 되고, 깔끔하게 지워도 됩니다. 헤더의 실제 시작은 A1 이 가리키는 B3 이기 때문입니다.
